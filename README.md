...
## Overview
In modern retail store execution, computer vision cameras monitor shelves in real time. However, a major architectural friction point arises when physical reality diverges from digital stock ledgers—specifically through **SKU format discrepancies** and **packaging integrity failures** (e.g., a torn multipack exposing loose units). 

This API Bouncer acts as an event-driven validation layer sitting between edge cameras and core enterprise systems. It intercepts these SKU format discrepancies, executes dynamic **BOM (Bill of Materials) decompositions**, dynamically calculates proportional markdown prices, enforces strict regional UK alcohol compliance laws, and dispatches handheld PDA / thermal printer tasks for instant shop-floor intervention.
...
* **Live SaaS Dashboard**: Features a mobile-friendly, real-time web dashboard rendering audit logs, SKU discrepancy alerts, packaging integrity status, and live pricing derivations.
